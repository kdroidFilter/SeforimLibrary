...
```bash
./gradlew :cli:run --args="search בראשית"
```
...
./gradlew :cli:createDistributable
# Output: cli/build/compose/binaries/main/app/

# Platform-specific installer
./gradlew :cli:packageDeb          # Linux .deb
./gradlew :cli:packageRpm          # Linux .rpm
./gradlew :cli:packageDmg          # macOS .dmg
./gradlew :cli:packageMsi          # Windows .msi

# All formats for current platform
./gradlew :cli:package
```

Output directory: `cli/build/compose/binaries/main/`
...
The CLI uses the same default paths as SeforimApp:
